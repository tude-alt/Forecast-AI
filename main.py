...
def extract_question_id(question: MetaculusQuestion) -> str:
    ...
    try:
        ...
        # handle both /questions/123 and /questions/123/
...
class ConservativeHybridBot(ForecastBot):
    ...
    def _llm_config_defaults(self) -> dict[str, str]:
        # FIX: include 'summarizer' so ForecastBot doesn't crash
        return {
            "default": "openrouter/openai/gpt-5.2",
            "parser": "openrouter/openai/gpt-4.1-mini",
            "researcher": "openrouter/openai/gpt-5.2",
            "summarizer": "openrouter/openai/gpt-5.2",  # <-- IMPORTANT FIX
        }
    ...
    # Committees (unchanged: you can keep your committee logic here)
    async def _run_forecast_on_binary(self, question: BinaryQuestion, research: str) -> ReasonedPrediction[float]:
        ...
        for model in models:
            try:
                ...
                notes.append(f"- model={model} p={float(p):.3f}")
            except Exception as e:
                fallback = clamp01(float(base) if isinstance(base, (int, float)) else 0.5)
                raw_ps.append(fallback)
                notes.append(f"- model={model} FALLBACK p={fallback:.3f} err={e}")
        ...
        if isinstance(base, (int, float)):
            # simple blend; you can keep your evidence-strength gating if you want
    ...
    async def _run_forecast_on_multiple_choice(self, question: MultipleChoiceQuestion, research: str) -> ReasonedPrediction[PredictedOptionList]:
        ...
        for model in models:
            try:
                ...
                notes.append(f"- model={model} ok")
            except Exception as e:
                n = len(question.options)
                per_model_maps.append({opt: 1.0 / n for opt in question.options})
                notes.append(f"- model={model} FALLBACK err={e}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        for tid in args.tournament_ids:
            ...
            # Slow down between tournaments to reduce Cloudflare/429
            for attempt in range(RETRY_MAX):
                try:
                    ...
                    msg = str(e).lower()
                    # if metaculus blocks, backoff hard
